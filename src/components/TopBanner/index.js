...
class TopBanner extends Component {
  ...
  render() {
    const sideItemEls = MOCK.sideItems.map((sideItem,idx) => {
      return (
        <li key={idx} className="side-item" onMouseEnter={() => { this.evtSideEnter(sideItem.type) }}>
          {sideItem.content}
        </li>
      )
    })

    const currentGoodEls = this.state.filterCurrGoods.map((currentGood, idx) => {
      let goodEls = currentGood.map((good, indx) => {
        const option = (<a className="goods-buy-link" href="item.buyUrl">选购</a>)
        return (
          <li key={indx} className="category-goods">
            <a className="goods-link" href={good.sourceUrl}>
              <img src={good.imgUrl} alt="" />
              <div className="text-name">
                {good.name}
              </div>
            </a>
            {good.buyStatus ? option : null}
          </li>
        )
      })
      return (
        <ul key={idx} className="category-items">
          {goodEls}
        </ul>
      )
    })
    ...
    const categoryDetail = (
      <div className="site-category-detail">
        {currentGoodEls}
      </div>
    )
    return (
      <div className="top-banner clearfix">
        <div className="site-category"
          onMouseLeave={this.evtSideLeave}>
          <div className="top-side-left">
            <ul className="side-left">
              {sideItemEls}
            </ul>
          </div>
          {this.state.goodsStatus ? categoryDetail : null}
        </div>
        <Slider banners={MOCK.banners}></Slider>
      </div>
    )
  }
  ...
  evtSideEnter = (currType) => {
    let filterGoods = [[],[],[],[],[],[]]
    MOCK[currType].forEach(function (item, index) {
      let goodsIndex = Math.floor(index / 6)
      filterGoods[goodsIndex].push(item)
    })
    this.setState({
      currGoods: MOCK[currType],
      filterCurrGoods: filterGoods,
      goodsStatus: true
    })
  }
